    ...
    public class LuaSystemsInspector : UnityEditor.Editor
    {
        ...
        void drawSystemsMonitor(string systemsName)
        {
            ...
            EntitasEditorLayout.BeginVerticalBox();
            {
                ...
                {
                    EditorGUILayout.LabelField("Total", string.Format("{0:0.000}", Convert.ToSingle(LuaSystems.GetProfile(systemsName)["executecostnow"]) * 1000));
                ...
                if (!EditorApplication.isPaused)
                {
                    ...
                    LuaTable profile = LuaSystems.GetProfile(systemsName);
                    if ((bool)LuaSystems.GetProfile(systemsName)["enable"])
                    {
                        addDuration(Convert.ToSingle(profile["executecostnow"]) * 1000, Convert.ToInt32(profile["executecount"]));
                    }
            ...
            foreach (var name in names)
            {
                if (name.ToLower().Contains(_systemNameSearchTerm.ToLower()))
                {
                    EntitasEditorLayout.BeginHorizontal();
                    {
                        ...
                        var avg = string.Format("Ø {0:0.000}", initOnly ? initCost : LuaSystems.GetAverageCost(name) * 1000).PadRight(9);
                        var min = string.Format("min {0:0.000}", initOnly ? initCost : Convert.ToSingle(profile["executecostmin"]) * 1000).PadRight(11);
                        var max = string.Format("max {0:0.000}", initOnly ? initCost : Convert.ToSingle(profile["executecostmax"]) * 1000);
        ...
        void addDuration(float duration, int executecount)
        {
            if (_executecount == executecount)
                return;
